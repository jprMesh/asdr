...
Another approach to spectrum sensing is through the use of match filters. Match filters are designed to maximize the signal to noise ratio given an input signal, which we will call $s_{in}$. To operate signal sensing through match filtering, prior knowledge of the reference signal (or $s_{ref}$) to be detected must be known. \sout{The reference signal will be correlated with the incoming signal, which means that the two signals are overlapped to determine whether any distinct similarities exist between those signals.}\textbf{More mathematical, explain what this actually means not in vague words} This process is described in equation \ref{} below. N represents the length of the reference signal, in samples. 
