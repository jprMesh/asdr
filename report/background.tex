...
The most fundamental method of non-cooperative sensing is energy detection. In this approach, the power spectral density (or PSD) of the received signal is taken.\textbf{REFERENCE} \textbf{DESCIBE PSD BETTER, more equations}The PSD represents the measure of a signal’s intensity in the frequency domain computed through the fast fourier transform of a signal. This is then bandpass filtered to contain only the frequency bands being watched. These frequency bands are then integrated, to determine how much energy is present in the band. If this passes a certain threshold, the frequency band is marked as occupied.
\begin{figure}[ht]
\centering
...
Another approach to spectrum sensing is through the use of matched filters. Matched filters are designed to maximize the signal to noise ratio (SNR) given an input signal, which we will call $s_{in}$.\textbf{REF -Narut} To operate signal sensing through matched filtering, prior knowledge of the reference signal (or $s_{ref}$) to be detected must be known. \sout{The reference signal will be correlated with the incoming signal, which means that the two signals are overlapped to determine whether any distinct similarities exist between those signals.}\textbf{More mathematical, explain what this actually means not in vague words} This process is described in equation \ref{} below. N represents the length of the reference signal, in samples. 
...
Where $R_x(t,\tau)$ is just the autocorrelation function of x at t with x at $\tau$. This function gives us an understanding of the times when the signal repeats, but is missing vital information of the repeated frequencies. By taking the fourier transform of $R_x^\alpha(\tau)$, we can get a better understanding of the frequencies in the Cyclostationary Process. This is denoted as $S_x^\alpha(f)$, and equals to: \textbf{REF -Narut} \par
\[ S_x^\alpha(f)=\int{-\infty}^{\infty} R_x^\alpha(\tau)exp(-j2\pi f\tau d\tau) \] \textbf{tried to fix the equation, check if correct -Narut}
This is called the Spectral Correlation Function (SCF). When it is normalized, it becomes the Spectral Coherence Function (SOF): \textbf{REF -Narut}
\[C_x^\alpha(f) = \frac{ S_x^\alpha(f)}{ (S_x^\alpha(f+\alpha/2) S_x^\alpha(f-\alpha/2))^{1/2}} \]
The SOF is from between 0 and 1, and represents the strength of the periodicity at that point. By plotting this value, the unique response of the Cyclostationary Process can be found, allowing for it to be categorized. \textbf{Plot? -Narut} \par
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%I'm up to here - Max
\section{Radio Localization \textbf{REF -Narut}}
...
\textbf{I know how useless this seems rn but probably want to go into some details about differences in kite types}\\
\textbf{Also maybe a picture?}\\
...
\textbf{A picture of fixed wing aircraft maybe?} \\
