...
\begin{table}[ht!]
	\centering
	\caption{This table gives an ourline on the L-STF Characteristics for an OFDM transmission. Due to our Wi-Fi signals only being in the 20, 40, and 80 MHz range, the OFDM match filter will use these values in order to perform spectrum sensing. The match filter can filter upon the subcarrier spacing of 312.5 kHz by noticing a change in the center frequency of a transmission.}	
