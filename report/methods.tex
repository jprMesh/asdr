...
In this chapter, details about the methods used in this project are discussed in detail. Energy detection and matched filtering are used for identifying if there is WiFi, localization occurs using the RSS method. As a result of the small size and cost-effectiveness of the GPS chosen, a Kalman filter was designed to increase accuracy of the measurements. The different considerations in communicating control messages to the drone and information messages to the ground without interfering with sensing were discussed as well. In addition, the different test protocols for the two different target regions were detailed.